...
export default function Logo({ to = "/home", compact = false, showBadge = true, inlineEar = true }) {
  return (
    <Link to={to} className={`logo-wrapper ${compact ? "compact" : ""}`} aria-label="ネコTV — home">
      {showBadge && (
        <span className="logo-badge" aria-hidden>
          <svg viewBox="0 0 64 64" width="28" height="28" xmlns="http://www.w3.org/2000/svg" role="img" aria-hidden>
      ...
      <h1 className="logo-text">
        <span className={`logo-char ne ${inlineEar ? "has-ear" : ""}`}>
          ネ
          {inlineEar && (
            <svg className="ear" viewBox="0 0 24 24" width="16" height="16" aria-hidden focusable="false">
